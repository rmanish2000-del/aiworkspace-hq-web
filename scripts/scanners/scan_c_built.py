# scan_c_built.py — the OPERATIONAL copy of Cowork's scan_c.py, repointed at
# the BUILT Astro output per WIRE-SCANS-TO-BUILT-OUTPUT (founder ruling
# 2026-08-13, ruling 5: "Repoint scan_c.py's MANIFEST to the Astro paths").
#
# Derivation from the verbatim original (scan_c.py, sha256
# 53d3e18330e9a57c5855565e7045ac3c86ba4eedda5e5e9f19fb4e4e8165b201, pinned in
# SHA256SUMS.txt beside this file):
#   1. MANIFEST now names the seven deployed dist/ documents — the artifact
#      that actually ships — instead of markdown sources and a standalone HTML.
#   2. Every surface is built HTML now, so the visible-text extraction the
#      original applied only to the landing page applies to all seven.
#   3. The extracted landing text is held in memory instead of writing
#      7-landing.txt into the scanned tree.
# The BANNED list (22 terms), SHIPPED_PHRASES (15), TRIGGERS, ATTRIB, the
# STANDING sentence and the scan() logic are UNCHANGED from the original.
# Exit codes are the original's: 0 clean, 1 findings, 2 SCANNER ABORT (a
# manifest surface did not resolve — a 2 is a false-green risk and CI must
# fail it louder than a 1).
...
def scan(path, text, is_page=True):
    ...
    for t in BANNED:
        for m in re.finditer(re.escape(t), text, re.I):
            ...
            if t == "return" and re.search(r"returns? to the|return takes effect|returns to", seg, re.I):
                continue
            if not NEG.search(c):
                f.append(("A/BANNED", t, c.strip().replace("\n", " ")[:120]))
    # ---- SCAN B
...
# ---- SURFACE MANIFEST — PINNED, NOT GLOBBED. The seven DEPLOYED documents.
ROOT = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..'))
MANIFEST = ["dist/terms.html", "dist/privacy.html", "dist/refunds.html",
            "dist/delivery.html", "dist/contact.html", "dist/about.html",
            "dist/warrant-guardian/index.html"]

missing = [m for m in MANIFEST if not os.path.isfile(os.path.join(ROOT, m))]
if missing:
...
def visible_text(raw):
    v = re.sub(r'<style.*?</style>|<script.*?</script>|<!--.*?-->', '', raw, flags=re.S)
...
for m in MANIFEST:
    ...
    res = scan(p, txt)
    print(f"\n=== {m}  ({len(txt.split())} words) ===")
    if not res:
        print("   CLEAN — A, B, C all pass")
    for kind, term, ctx in res:
        print(f"   [{kind}] {term}\n        {ctx}")
    total += len(res)
print(f"\n{'=' * 60}\nSURFACES SCANNED: {len(MANIFEST)} of 7 required\nTOTAL FINDINGS: {total}")
