...
for (const route of ROUTES) {
  test(`${route.path} renders with its approved title and single h1`, async ({
    page,
  }) => {
    const response = await page.goto(route.path);
    expect(response?.status(), `${route.path} did not return 200`).toBeLessThan(
      400,
    );
  ...
  test(`${route.path} has the landmark map and no skipped heading level`, async ({
    page,
  }) => {
    await page.goto(route.path);
  ...
  test(`${route.path} emits a self-referencing canonical and the right robots directive`, async ({
    ...
    const expected =
      route.path === "/"
        ...
        : `https://aiworkspacehq.com${route.path}`;
    ...
    const expectedRobots = WAVE_1.includes(route.path)
      ? /^index, follow$/
      : /noindex/;
  ...
  test(`${route.path} leaks no build-time placeholder and ships no script`, async ({
    page,
  }) => {
    await page.goto(route.path);
    ...
    expect(html, `[LEGAL] marker leaked on ${route.path}`).not.toContain(
      "[LEGAL",
    );
...
test("the nav carries the specified items in order", async ({ page }) => {
  ...
  const labels = await page
    .locator('nav[aria-label="Main"] li')
    .allInnerTexts();
...
test("the active route is marked with aria-current on every route", async ({
  page,
}) => {
...
test("the wordmark links home on every route except home", async ({ page }) => {
  ...
  for (const path of [
    ...
    await page.goto(path);
    await expect(page.locator("header a.wordmark"), path).toHaveAttribute(
      "href",
      "/",
    );
  }
...
test("no route links to /docs or /research anywhere", async ({ page }) => {
  for (const route of ROUTES) {
    ...
    for (const href of hrefs) {
      expect(href, `${route.path} links to a deferred route`).not.toMatch(
        /\/(docs|research)\b/,
      );
    }
  }
...
test("every internal link resolves to a route that exists", async ({
  page,
  request,
}) => {
...
test("the sitemap lists exactly the indexable routes", async ({ request }) => {
  ...
  expect(locs).toEqual(
    SITEMAP_ROUTES.map((r) =>
      r.path === "/"
        ? "https://aiworkspacehq.com/"
        : `https://aiworkspacehq.com${r.path}`,
    ),
  );
...
test("Organization JSON-LD appears on / only, and asserts nothing prohibited", async ({
  page,
}) => {
  await page.goto("/");
...
test("the superseded 404 string appears nowhere in the build", async ({
  page,
}) => {
...
test("the five principles render on /principles; the landing route follows its contract", async ({
  ...
  const onPage = await page
    .locator(".term-list--heading .term-list__term")
    .allTextContents();
...
test("/platform preserves intent tense and renders only verified Assignment capabilities", async ({
  ...
  const pillars = await page
    .locator(".term-list--heading .term-list__body")
    .allTextContents();
  ...
  await expect(capability.locator(".ledger-badge")).toContainText(
    "VERIFIED · 2026-08-07",
  );
  ...
  await expect(routingCapability.locator(".ledger-badge")).toContainText(
    "VERIFIED · 2026-08-07",
  );

  const activationCapability = page.locator('[data-block="CB-82"]');
  await expect(activationCapability).toContainText(
    "properly routed Assignment",
  );
  ...
  await expect(completionCapability).toContainText(
    "evidence-backed submitted result",
  );
  await expect(completionCapability).toContainText(
    "deterministic verification",
  );
  await expect(completionCapability).toContainText(
    "authorized human approval or rejection",
  );
  await expect(completionCapability).toContainText(
    "Remediable failures create a new Attempt",
  );
  await expect(completionCapability).toContainText(
    "remain inspectable and unchanged",
  );
  ...
  await expect(knowledgeCapability).toContainText(
    "ingest governed organizational knowledge",
  );
  ...
  await expect(knowledgeCapability.locator(".ledger-badge")).toContainText(
    "VERIFIED · 2026-08-08",
  );
...
test("/contact publishes no address and no form", async ({ page }) => {
  ...
  for (const withheld of ["General enquiries", "Where we are"]) {
    expect(html, `${withheld} still reaches the document`).not.toContain(
      withheld,
    );
  }
});

test("/privacy renders all twelve sections in order, with no leaked text", async ({
  page,
}) => {
