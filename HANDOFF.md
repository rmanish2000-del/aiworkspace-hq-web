...
> **Cross-workstream authorisation (rule 4), 2026-08-18 —
> HOME-CTA-AND-BETA-PATH:** by Chat ruling on founder input, the web session
> edited the Codex-owned `src/content/copy.ts`: home primary CTA unified to
> "View verified evidence" (nav string; evidence-first positioning), platform
> path moved to the secondary slot, and a small "See pricing" link added to
> the hero action row. Hero heading, palette, positioning and the C-11 stage
